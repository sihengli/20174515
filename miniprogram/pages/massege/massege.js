...
Page({
  ...
  onMassege: function(e) {
    var massege = this.data.massege
    var id = e.currentTarget.dataset.id
    console.log(e.currentTarget.dataset.id)
    if (massege[id].class1 == '通知') {
      wx.navigateTo({
        url: '../notice/notice-detail/notice-detail?notice=' + JSON.stringify(massege[id])
      })
    } else {
      wx.navigateTo({
        url: '../lostAndFound/detail/detail?post=' + JSON.stringify(massege[id])
      })
    }
  ...
  choosemassege: function(e) {
    var delete1 = wx.getStorageSync('demassege');
    var p1 = 20;
    var p2 = 20;
    var i = 0;
    var t = 0;
  ...
  onRemove: function(e) {
    var massege = this.data.massege;
    massege.splice(e.currentTarget.dataset.id, 1)
    console.log(e.currentTarget.dataset.id);
    this.setData({
      massege: massege
    })
    ...
    if (!delete1) {
      ...
      wx.setStorageSync('demassege', newdelete1)
    } else {
      delete1.unshift(e.currentTarget.dataset.viewid);
      if (delete1.length > 20) {
        delete1.pop()
      }
      wx.setStorageSync('demassege', delete1)
    }
    // console.log(delete1);
